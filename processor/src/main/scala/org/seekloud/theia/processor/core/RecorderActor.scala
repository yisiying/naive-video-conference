...
//import org.seekloud.theia.processor.core.GrabberManager.RecorderRef
//import org.seekloud.theia.processor.Boot.{channelWorker, executor, grabberManager, recorderManager, scheduler, timeout}
//import akka.actor.typed.scaladsl.AskPattern._
//
//import scala.concurrent.duration._
//import scala.util.{Failure, Success, Try}
//import org.seekloud.theia.processor.protocol.SharedProtocol.RoomInfo
//import org.seekloud.theia.processor.common.AppSettings._
//import org.bytedeco.ffmpeg.global.{avcodec, avutil}
//import org.seekloud.theia.processor.core.ChannelWorker.GetTs
...
//          }
//          //每创建一个RecorderActor，建立一个FFmpegFrameRecorder(out,1280,720,2)
//          //其中out为一个PipeStream的输入端，一个roomId对一个一条管道
//          //每创建一个RecorderActor，就将自己的引用发送至grabbermanager
...
//          val canvas = new BufferedImage(Math.max(width, 640), Math.max(height, 360), BufferedImage.TYPE_3BYTE_BGR)
//          val canvas4Ts = new BufferedImage(Math.max(width, 640), Math.max(height, 360), BufferedImage.TYPE_3BYTE_BGR)
//          val drawer = ctx.spawn(draw(canvas, canvas.getGraphics, Ts4LastImage(), Image(), recorder4ts,
//            new Java2DFrameConverter(), new Java2DFrameConverter(), layout, "defaultImg.jpg", roomId, (Math.max(width, 640), Math.max(height, 360))), s"drawer_$liveId")
//          single(roomId, canvas4Ts, canvas4Ts.getGraphics, new Java2DFrameConverter(), Ts4LastImage(), Ts4LastSample(), host, client, layout, aiMode, recorder4ts, ffFilter, drawer, out, tsDifer, (Math.max(width, 640), Math.max(height, 360)))
//
//
//          //处理音画同步，添加时间戳
