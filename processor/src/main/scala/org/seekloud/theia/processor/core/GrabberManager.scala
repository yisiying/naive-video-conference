...
//import org.seekloud.theia.processor.Boot.blockingDispatcher
//
//import scala.language.implicitConversions
//import scala.collection.mutable
//import scala.concurrent.duration._
//import org.seekloud.theia.processor.Boot.{channelWorker, recorderManager}
//import org.seekloud.theia.processor.core.GrabberActor.{GrabFrame, StartGrabber}
...
//      msg match {
//          //创建一个RecorderRef即往该处发送一个RecorderRef，包含改引用和roomId
//        case m@RecorderRef(roomId, ref) =>
//          log.info(s"got msg: $m")
//          //根据roomID获取到GrabberActor，一个roomId对应一个list，list中最多存两个引用，一个主播，一个连线
//          val grabbers = roomRoleRefMap.get(roomId)
//          //如果role非空
//          if(grabbers.isDefined){
//            grabbers.get.foreach {g=>
//              //拿到grabberactor的引用g(主播、连线)，向其发送Recorder(ref)，即recorderActor的引用
...
//          roomRoleRefMap.get(roomId).foreach{_.foreach{g=> g ! GrabberActor.Stop}}
//          roomRoleLiveMap.get(roomId).foreach(lives=> lives.foreach(l=> timer.startSingleTimer(Timer4Pipe(l), ClosePipe(l), 100.milli)))
