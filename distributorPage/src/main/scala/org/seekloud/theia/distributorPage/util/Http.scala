package org.seekloud.theia.distributorPage.util
...
object Http {
  ...
  lazy val jsonPostHeader: Headers = {
    try {
      val h = new Headers()
  ...
  lazy val formPostHeader: Headers = {
    try {
      val h = new Headers()
...
//        headers = formPostHeader,
  ...
  def postFormDataXML(url: String, bodyStr: FormData, withCookie: Boolean = true): Future[String] = {
    println(s"sendJsonPost: url=$url body=$bodyStr header=$formPostHeader")
    try {
      val requestInit = RequestInit(
        method = HttpMethod.POST,
  ...
  def postFormAndParseXML[T](
                           url: String,
                           bodyStr: FormData,
                           withCookie: Boolean = true)(implicit decoder: Decoder[T]): Future[Either[Error, T]] = {
    import io.circe.parser._
    postFormDataXML(url, bodyStr, withCookie).map(s => {println("ssss is",s);decode[T](s) })
  }
...
//        println(s"i.blob=====${i.blob()}")
        i.blob().toFuture.map{i =>
          try {
            println(i)
            println(i.size)
            val url: Unit = file.readAsDataURL(i)
