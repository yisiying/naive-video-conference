...
object TimeTool {
  ...
  def parseDateLikeBiliBili(timestamp: Long): String ={
    val timeNow = new Date().getTime()
    val timeLong = (timeNow - timestamp).toInt
    timeLong match{
      case a if a < 60000 => "刚刚"
      case a if a < 3600000 => s"${a/60000}分钟前"
      case a if a < 86400000 => s"${a/3600000}小时前"
      case a if a < 172800000 => "昨天"
      case a if a < 259200000 => "前天"
      case _ => dateFormatDefault(timestamp, "yyyy-MM-dd")
    }
  }
