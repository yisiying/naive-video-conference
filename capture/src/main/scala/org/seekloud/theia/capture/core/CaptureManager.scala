...
object CaptureManager {
  ...
  private val latestDesktopFrame = new java.util.concurrent.LinkedBlockingDeque[Messages.LatestFrame](1)
  ...
  def setLatestDesktopFrame(): Unit = latestDesktopFrame.clear()
      ...
      msg match {
        ...
        case StartCapture =>
          ...
          val imageCaptureOpt = if (grabber.nonEmpty) {
            Some(getImageCapture(ctx, latestFrame, grabber.get, montageActor, mediaSettings.frameRate, debug))
          } else None
  ...
  private def idle(
    ...
    Behaviors.receive[Command] { (ctx, msg) =>
      msg match {
        ...
        case AskFrame =>
          val targetFrame = latestFrame.peek()
          if (targetFrame != null) {
...
//            log.info(s"No frame captured yet.")
        ...
        case AskDesktopFrame =>
          val targetFrame = latestDesktopFrame.peek()
          if (targetFrame != null) {
...
//            log.info(s"No desktop frame captured yet.")
        ...
        case CameraGrabberStarted(grabber1, only) =>
          ...
          val imageCaptureOpt1 =
            Some(getImageCapture(ctx, latestFrame, grabber1, montageActor, mediaSettings.frameRate, debug))
          idle(
            replyTo,
            mediaSettings,
            grabber,
        ...
        case DesktopGrabberStarted(grabber1, only) =>
          ...
          val desktopCaptureOpt1 =
            Some(getDesktopCapture(ctx, latestDesktopFrame, grabber1, montageActor, mediaSettings.frameRate, debug))
          idle(
            replyTo,
            mediaSettings,
            grabber,
  ...
  private def getImageCapture(
    ...
    ctx.child(childName).getOrElse {
      val actor = ctx.spawn(ImageCapture.create(grabber, latestFrame, frameRate, debug, montageActor), childName, blockingDispatcher)
  ...
  private def getDesktopCapture(
    ...
    ctx.child(childName).getOrElse {
      val actor = ctx.spawn(DesktopCapture.create(grabber, latestDesktopFrame, frameRate, debug, montageActor), childName, blockingDispatcher)
      ctx.watchWith(actor, ChildDead(childName, actor))
      actor
